...
<a href="index.asp">JLL</a> &gt; <a href="3.asp">个人委托</a>
</div>
<!--广告部份-->
...
<div class="container">
  ...
  '  response.write sql
    ...
    %>
    <li><a href="6.asp?id=<%=rs("articleid")%>" class="fpic"><!--<i class="hot"></i>--><img src="<%=rs("defaultpicurl")%>" class="fangpic b_h"></a><div class="ninfo"><a  href="6.asp?id=<%=rs("articleid")%>" class="ntit b_h"><%=rs("title")%></a><span class="ntxt"><span class="ntxtl">区域<br>户型<br>豪宅类型<br></span><span class="ntxtr"><%=rs("bigclassname")%>，<%=rs("smallclassname")%><br>
<%
...
<% end if
 next %><br><%=rs("clbname")%><br></span>
    ...
    </div><div class="nphone"><span class="jiage">¥<%=rs("jgzj")%><b class="b1">万起</b><!--<b class="b2">(每平)</b>--></span><div class="c"></div><a href="6.asp?id=<%=rs("articleid")%>" class="nbtn">查看更多</a></div></li>
<%
rs.movenext
i=i+1
loop
