...
$(".save").click(function(){
	CName=$("#XGCName").val();
	CNameen=$("#XGCNameen").val();
	CID=$("#XGCID").val();
	CFile=$("#XGCFile").val();
	Action=$("#XGAction").val();
	OID=$("#XGOID").val();
	Table=$("#XGTable").val();
		if (Action=="del"){$(this).attr("data-dismiss","modal");XgClass(CName,CNameen,Action,CID,OID,Table,CFile);}
	else{
 ...
 XgClass(CName,CNameen,Action,CID,OID,Table,CFile);//执行修改操作
/*		  console.log(CID);
		  console.log(CName);
		  console.log(Action);
		  console.log(OID);
...
//              输出类别列表         //
////////////////////////////////////
function XgClass(CName,CNameen,Action,CID,OID,Table,CFile){
console.log(Table+'2');
//ajax操作数据库
	 $.post("ajax/xgclass.asp",{CID:CID,CName:CName,CNameen:CNameen,Action:Action,OID:OID,Table:Table,CFile:CFile},
	...
	str+='var cnameen=$(this).data("cnameen");';
...
str+='$("#xg-class").before("<p>是否确定要删除以下类别及相关产品信息吗？</p>");'
	...
	for (var i=0;i<jsObjstr.length;i++)
					{
	str+='<tr style="background-color:#F9F9F9;"><td style="padding-left:50px;"><img src="xgwl/img/jia.gif" width="15" height="15">'+jsObjstr[i].CName+'</td><td>'+jsObjstr[i].OID+'</td><td><button class="btn btn-success btn-xs mod" data-action="mod" data-cid="'+jsObjstr[i].CID+'" data-cname="'+jsObjstr[i].CName+'" data-cnameen="'+jsObjstr[i].CNameen+'" data-cfile="'+jsObjstr[i].CFile+'" data-toggle="modal" data-oid="'+jsObjstr[i].OID+'" data-xgtable="'+Table+'">修改</button> <button class="btn btn-danger btn-xs mod" data-action="del" data-cid="'+jsObjstr[i].CID+'" data-cname="'+jsObjstr[i].CName+'" data-cnameen="'+jsObjstr[i].CNameen+'" data-toggle="modal" data-oid="'+jsObjstr[i].OID+'" data-xgtable="'+Table+'">删除</button></td></tr>';
}
	str+='</table>';
//	
